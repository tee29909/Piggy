...
const moneyController = {
  addMoney: (req, res) => {
    ...
    Money.create({ userID: payload.userID, money: payload.money })
      .then(function (money) {
        res.json(money);
      })
      .catch(function (err) {
  ...
  updateMoney: (req, res) => {
    const payload = req.body;
    Money.updateOne({ _id: payload._id }, payload)
      .then(function (money) {
        res.json(money);
      })
      .catch(function (err) {
  ...
  getMoneyByUserID: (req, res, next) => {
    const { id } = req.params;
    Money.findOne({ userID: id })
      .then(function (money) {
        res.json(money);
      })
      .catch(function (err) {
